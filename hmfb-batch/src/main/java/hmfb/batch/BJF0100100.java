...
import hmfb.batch.service.F0100100Service;
 ...
 */
@Log4j2
public class BJF0100100 implements IChunkBatchJob {
...
//		일요일엔 정상 종료 리턴. (배치를 수행시키지 않는다. 상태는 FAILED 가 아닌 COMPLETED)
	...
	public T0100100Dto process(Object param, BatchJobContext ctx) throws HmfbException {
		...
		F0100100Dto inFirmDto = new F0100100Dto();
		
		FirmReturnDto returnDto = F0100100Service.getService(F0100100Service.class).f0100100Service(inFirmDto);
		...
		if (log.isDebugEnabled()) {
			log.debug("F0100100 전문 응답 처리 완료");
