...
/**
 * 거래명세통지정보(tcp 수신 테스트 용)
 * INPUT 데이터  : 
 * OUTPUT     : 거래명세통지정보 인터페이스  
 *  
 * @author KDK
 *
 */
@Log4j2
public class BJF0600100 implements IChunkBatchJob {
...
//		DB 조회 조건을 입력
	 ...
	 */
	@Override
	public T0600100Dto process(Object param, BatchJobContext ctx) throws HmfbException {
		
		T0600100Dto input = (T0600100Dto) param;
		T0600100Dto output = new T0600100Dto();
		
		F0600100Dto inFirmDto = new F0600100Dto();
		...
		FirmReturnDto returnDto = F0600100Service.getService(F0600100Service.class).f0600100Service(inFirmDto, input.getTelemsgNo());
		//F0600100Dto outFirmDto = (F0600100Dto) returnDto.getRtnObj();
		...
		output.setSendYn("Y");
		BatchDao.getDao().update("T0600100.updateT0600100", output);
		if (log.isDebugEnabled()) {
			log.debug("F0600100 전문 응답 처리 완료");
