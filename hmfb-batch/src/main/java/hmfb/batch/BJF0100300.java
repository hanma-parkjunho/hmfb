...
import hmfb.batch.service.F0100300Service;
 ...
 */
@Log4j2
public class BJF0100300 implements IChunkBatchJob {
...
//		일요일엔 정상 종료 리턴. (배치를 수행시키지 않는다. 상태는 FAILED 가 아닌 COMPLETED)
	...
	public T0100100Dto process(Object param, BatchJobContext ctx) throws HmfbException {
		...
		F0100300Dto inFirmDto = new F0100300Dto();
		
		FirmReturnDto returnDto = F0100300Service.getService(F0100300Service.class).f0100300Service(inFirmDto);
		...
		if (log.isDebugEnabled()) {
			log.debug("F0100300 전문 응답 처리 완료");
